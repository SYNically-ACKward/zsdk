...
def test_url_filtering_rules_get(tenant):
    filtering_rule = random.choice([rule.get('id') for rule
                                    in tenant.url_filtering_rules.list()])
...
def test_url_filtering_rules_update(tenant):
    rule = [rule for rule in tenant.url_filtering_rules.list() if rule.get('name') == "Pytest URLF Rule"]
...
def test_url_filtering_rules_delete(tenant):
    rule = [rule for rule in tenant.url_filtering_rules.list() if rule.get('name') == "Pytest URLF Rule - Updated"]
